...
describe("inquiries repository seed fallback", () => {
  it("returns inquiry fixtures in descending created order without a Supabase client", async () => {
    const { inquiriesModule, createSupabaseAdminClient } = await loadSeedFallbackModule();

    const inquiries = await inquiriesModule.getInquiryFixtures();

    expect(inquiries.map((item) => item.id)).toEqual(["inq-demo-001", "inq-demo-002", "inq-demo-003"]);
  ...
  it("resolves inquiry details by id and returns null for missing fixtures", async () => {
    ...
    await expect(inquiriesModule.getInquiryById("inq-demo-002")).resolves.toMatchObject({
      id: "inq-demo-002",
      status: "contacted",
      adminMemo: "1차 연락 완료",
      regionLabel: "서울 강동구"
  ...
  it("updates fixture-backed inquiries and stamps updatedAt with the current time", async () => {
    ...
    await expect(inquiriesModule.getInquiryById("inq-demo-001")).resolves.toMatchObject({
      status: "consulted",
      adminMemo: "2차 상담 완료",
      updatedAt: "2026-03-13T00:00:00.000Z"
    });
  ...
  it("returns 404 when updating a missing inquiry fixture", async () => {
    ...
    await expect(inquiriesModule.updateInquiry("missing", { status: "closed" })).resolves.toMatchObject({
      success: false,
      statusCode: 404,
  ...
  it("returns user-facing labels for each inquiry status", async () => {
    ...
    expect(inquiriesModule.getInquiryStatusLabel("in_progress")).toBe("신청진행");
