...
import {
  CalculatorHeaderOverride,
  CalculatorPriceOverride,
} from "@/components/sections/CalculatorPriceOverride";
...
export default async function ProductDetailPage({
  ...
  const productJsonLd = {
    ...
    brand: { "@type": "Brand", name: "인터넷공룡" },
    offers: {
      "@type": "Offer",
      priceCurrency: "KRW",
      price: product.monthlyPriceLabel,
    ...
    <>
          ...
          <div className="surface-card bg-white">
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              {product.badgeTags.map((tag) => (
                <span
                  key={tag}
                  className="rounded-full bg-brand-lavender-soft px-3 py-1 text-xs font-semibold text-brand-graphite"
                >
                  {tag}
                </span>
              ))}
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-3">
              ...
              >
                <CalculatorPriceOverride
                  fallbackSpeed={product.internetSpeed}
                  fallbackPrice={product.monthlyPriceLabel}
                />
              </Suspense>
              <div className="rounded-2xl bg-brand-surface p-4">
          ...
          <div className="surface-card bg-white">
            ...
            <ul className="mt-4 space-y-3 text-sm leading-6 text-brand-slate">
              {product.heroPoints.map((point) => (
                <li
                  key={point}
                  className="rounded-2xl bg-brand-lavender-soft px-4 py-3 text-brand-graphite"
                >
                  {point}
                </li>
              ))}
            </ul>
            <p className="mt-6 rounded-2xl bg-brand-sky-soft px-4 py-3 text-sm text-brand-graphite">
      ...
      <section className="py-8 md:py-12">
        <div className="container-page">
          <SectionHeading
            title="상품 안내"
            description="기획안 기준으로 추천 대상, 비교 포인트, 유의사항을 블록형 섹션으로 확장 가능하게 설계했습니다."
          />
      ...
      <section id="inquiry" className="py-8 md:py-12 bg-brand-lavender-soft/40">
        <div className="container-page grid gap-5 md:grid-cols-[0.9fr_1.1fr]">
          <div className="space-y-4">
            <SectionHeading
              title="이 상품 기준으로 상담받기"
              description="상품 상세 폼은 공통 필드 + payload 확장 구조로 설계해 후속 운영 시 필드 추가가 쉽습니다."
            />
            <div className="surface-card">
              <h3 className="text-lg font-bold text-brand-graphite">
                자주 묻는 질문
              </h3>
